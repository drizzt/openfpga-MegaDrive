#!/usr/bin/env bash
# Check a build's timing and write a report. With no baseline file any negative
# slack fails; with one, only a loss beyond the tolerances below does.
#
# Usage: check_timing_regression.sh [sta.summary] [baseline] [output.txt]
#
# pr.yml greps the first line of the output for "TIMING OK".
set -euo pipefail
...
# WNS in ns, TNS as a fraction. Sized off measured fitter noise: renaming the RTL
# directory, no logic changed, moved WNS 0.145 ns and TNS 1.3%
WNS_TOL=0.250
TNS_TOL=0.10
