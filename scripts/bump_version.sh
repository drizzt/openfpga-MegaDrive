...
#
# Sibling cores sharing one bitstream release together, so their versions are
# kept identical; gateware.json carries the same number for the gateman manifest.
# The current version is read from the first pkg/Cores/*/core.json (they are all
# in sync).
set -euo pipefail
